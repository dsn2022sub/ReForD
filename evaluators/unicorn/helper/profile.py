...
class BestClusterGroup():
    ...
    def optimize(self, arrs, distance, method='hamming', max_cluster_num=6, num_trials=20, max_iterations=1000):
        ...
        max_cluster_num = min(arrs.shape[0],
                              max_cluster_num)  # max_cluster_num should never be bigger than the total number of arrays
        for num_clusters in range(2, max_cluster_num):
            for trial in range(num_trials):
                ...
                actual_num_cluster = len(
                    medoids)  # @num_clusters is the maximum possible cluster. The actually number (@actual_num_cluster) may be smaller.
                ...
                if silhouette_coef > best_silhouette_coef or (
                        silhouette_coef == best_silhouette_coef and actual_num_cluster > self.best_num_clusters):  # We prefer larger cluster number.
...
class Model():
    ...
    def __init__(self, name, medoids=None, members=None, mean_thresholds=None, max_thresholds=None, stds=None,
                 evolution=None):
        """
    ...
    def construct(self, arrs, dists, best_cluster_group):
        ...
        for cluster_idx in range(best_num_clusters):
            ...
            self.members[cluster_idx] = best_medoids[
                cluster_idx].elements  # elements is a list of indices of all the members in the cluster
            # cluster_dists contains all the distances between the kernel (medoid) and each member array
            cluster_dists = [dists[cluster_centers[cluster_idx]][elem] for elem in self.members[cluster_idx] if
                             elem != cluster_centers[cluster_idx]]
...
def test_single_graph(arrs, models, metric, num_stds, debug_info=None):
    ...
    for model in models:
        ...
        if metric == 'mean':
            current_threshold = model.get_mean_thresholds()[current_cluster_idx] + num_stds * model.get_stds()[
                current_cluster_idx]
        elif metric == 'max':
            current_threshold = model.get_max_thresholds()[current_cluster_idx] + num_stds * model.get_stds()[
                current_cluster_idx]

        for arr_id, sketch in enumerate(arrs):
            distance_from_medoid = hamming(sketch,
                                           current_medoid)  # Compute the hamming distance between the current medoid and the current test sketch.
            if distance_from_medoid > current_threshold:
                ...
                if current_evolution_idx < len(model.get_evolution()) - 1:  # If there is a next cluster in evolution.
                    ...
                    current_medoid = model.get_medoids()[current_cluster_idx]
                    if metric == 'mean':
                        current_threshold = model.get_mean_thresholds()[current_cluster_idx] + num_stds * \
                                            model.get_stds()[current_cluster_idx]
                    elif metric == 'max':
                        current_threshold = model.get_max_thresholds()[current_cluster_idx] + num_stds * \
                                            model.get_stds()[current_cluster_idx]
