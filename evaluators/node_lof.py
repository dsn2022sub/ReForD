...
def load_data(path, index):
    ...
    normal_base_path = path + '/normal/'
    # attack_stram_path = path+'/attack/filtered-stream/'
    attack_base_path = path + '/attack/'
    y_true = []
    G = nx.Graph()
    G1 = nx.Graph()
    '''
    ...
    path2 = normal_base_path + 'all-wget-normal-' + str(index) + '.txt'
    ...
    if index <= 24:
        path3 = attack_base_path + 'all-wget-attack-' + str(index) + '.txt'
...
for i in range(125):
    ...
    # print('fit begin')
    # model1 = node2vec1.fit(window=3, min_count=1, batch_words=4)  # Any keywords acceptable by gensim.Word2Vec can be passed, `dimensions` and `workers` are automatically passed (from the Node2Vec constructor)
    # for word in model1.wv.index_to_key:
    #    x.append(list(model1.wv[word]))
    # print(x)
    ...
    if i <= 24:
        ...
        # model2 = node2vec2.fit(window=3, min_count=1, batch_words=4)  # Any keywords acceptable by gensim.Word2Vec can be passed, `dimensions` and `workers` are automatically passed (from the Node2Vec constructor)
        # for word in model2.wv.index_to_key:
        #    x1.append(list(model2.wv[word]))
