...
def plot_with_labels(lowDWeights, labels):
    ...
    for x, y,z, s in zip(X, Y,Z, labels):
        ...
        plt.text(x, y,z, s, backgroundcolor=c, fontsize=9)
...
if __name__ == '__main__':
    ...
    with open('coordinates.txt', 'w') as co_fh:
        for emb in all_embs:
            co_fh.write(str(emb[0]) + ' ' + str(emb[1]) + ' ' + str(emb[2]) + '\n')
    with open('labels.txt', 'w') as lb_fh:
...
# defined by x in [23, 32], y in [0, 100], z in [zlow, zhigh].
    ...
    X, Y ,Z= all_embs[:, 0], all_embs[:, 1],all_embs[:, 2]
    for x, y,z, s in zip(X, Y,Z, all_labels):
        c = cm.rainbow(int(255/9 * s))
    ...
    #plt.show()
    #plotutils.plot(all_embs,all_labels, colors=plotutils.MOUSE_10X_COLORS)
    #plot_with_labels(all_embs, all_labels)
